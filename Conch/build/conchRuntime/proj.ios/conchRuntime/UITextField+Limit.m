...
@interface _LimitInfoView : NSObject

@property(nonatomic,weak)id<UITextViewDelegate> pinocchio;
...
@interface UITextViewDelegateManager : NSObject<UITextViewDelegate> {
    NSMapTable<id,_LimitInfoView *> *_infos;
    pthread_mutex_t _mutex;
}

+ (instancetype)sharedInstance;

- (void)addLimitNums:(NSInteger)num key:(id)key target:(id<UITextViewDelegate>)target action:(void(^)(void))action;

- (void)addLimitCondition:(BNConditionBlock)condition key:(id)key target:(id<UITextViewDelegate>)target action:(void(^)(void))action;

- (void)observeValueWithCondition:(BNConditionBlock)condition key:(id)key target:(id<UITextViewDelegate>)target action:(void (^)(void))action;
...
- (void)bn_removeFromSuperview{
    [UITextViewDelegateManager.sharedInstance removeLimitForKey:self];
    return [self bn_removeFromSuperview];
}

- (void)limitNums:(NSInteger)num action:(void (^)(void))action{
    
    [[UITextViewDelegateManager sharedInstance] addLimitNums:num
                                                          key:self
                                                       target:self.delegate
                                                       action:action];
    self.delegate =  UITextViewDelegateManager.sharedInstance;
#pragma clang diagnostic push
#pragma clang diagnostic ignored "-Wundeclared-selector"
    

//    [self addTarget:UITextViewDelegateManager.sharedInstance action:@selector(textViewDidChanged:) forControlEvents:UIControlEventEditingChanged];
#pragma clang diagnostic pop

}

- (void)limitCondition:(BNConditionBlock)condition action:(void (^)(void))action{
    [[UITextViewDelegateManager sharedInstance] addLimitCondition:condition
                                                               key:self
                                                            target:self.delegate
                                                            action:action];
    self.delegate =  UITextViewDelegateManager.sharedInstance;
...
- (void)observeValueWithCondition:(BNConditionBlock)condition action:(void(^)(void))action{
    [[UITextViewDelegateManager sharedInstance] observeValueWithCondition:condition
                                                                       key:self
                                                                    target:self.delegate
                                                                    action:action];
}
...
@implementation UITextViewDelegateManager
...
- (void)addLimitWithDisturb:(BOOL)disturb andCondition:(BNConditionBlock)condition andNums:(NSInteger)num key:(id)key target:(id<UITextFieldDelegate>)target action:(void (^)(void))action{
    ...
    pthread_mutex_lock(&_mutex);
    _LimitInfoView *info = [_infos objectForKey:key];
    
    if (!info) {
        info = [_LimitInfoView new];
        info.pinocchio = target;
    }
    if (disturb) {
        if (condition) {
...
- (void)removeLimitForKey:(id)key{
    ...
    pthread_mutex_lock(&_mutex);
    _LimitInfoView* info = [_infos objectForKey:key];
    ((UITextView*)key).delegate = info.pinocchio;
    [_infos removeObjectForKey:key];
    pthread_mutex_unlock(&_mutex);
}


- (_LimitInfoView*)safeReadForKey:(id)key{
    if (!key) {
        return nil;
    }
    pthread_mutex_lock(&_mutex);
...
- (void)textViewDidChanged:(UITextView *)textView {
    ...
    _LimitInfoView *info = [self safeReadForKey:textView];
    if (info.num != 0) {
        if (info && textView.text.length >= info.num > 0) {
            textView.text = [textView.text substringToIndex:info.num];
            info.action();
        }
    }
}
...
- (BOOL)textView:(UITextView *)textView shouldChangeTextInRange:(NSRange)range replacementText:(NSString *)string{
    ...
    if (info.num != 0) {
        if (info && textView.text.length == info.num && string.length > 0) {
            info.action();
            checkInLimit = YES;
        }
    }
    ...
    return [info.pinocchio textView:textView shouldChangeTextInRange:range replacementText:string];
}
...
- (BOOL)textViewShouldBeginEditing:(UITextView *)textView{
    ...
    if ([info.pinocchio respondsToSelector:@selector(textViewShouldBeginEditing:)]) {
        return [info.pinocchio textViewShouldBeginEditing:textView];
    }
    return YES;
}
- (void)textViewDidBeginEditing:(UITextView *)textView{
    _LimitInfoView *info = [self safeReadForKey:textView];
    if ([info.pinocchio respondsToSelector:_cmd]) {
        return [info.pinocchio textViewDidBeginEditing:textView];
    }
}
- (BOOL)textViewShouldEndEditing:(UITextView *)textView{
    ...
    return [info.pinocchio textViewShouldEndEditing:textView];
}
- (void)textViewDidEndEditing:(UITextView *)textView{
    _LimitInfoView *info = [self safeReadForKey:textView];
    if ([info.pinocchio respondsToSelector:_cmd]) {
        return [info.pinocchio textViewDidEndEditing:textView];
    }
}
